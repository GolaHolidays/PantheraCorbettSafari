...
export const Header: React.FC<HeaderProps> = ({
  ...
  const navLinks = [
    ...
    { label: "Night Stay (FRH)", href: "/#night-stays" },
    { label: "Jeep vs Canter", href: "/#safari-types" },
    { label: "Cabs & Hotels", href: "/#secondary-services" },
    { label: "Permit FAQ", href: "/#faq" },
  ];

  return (
    <header className="sticky top-0 z-40 bg-[#17211A] text-[#FBF8F0] border-b border-[#37482E]">
      {/* Top trust notification strip */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <Link href="/" className="flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-[4px] bg-[#37482E] border border-[#8A9468]/40 flex items-center justify-center text-[#C99A3D] font-serif font-bold text-xl group-hover:border-[#C99A3D] transition-colors">
              P
            </div>
            <div>
              <div className="font-serif text-xl sm:text-2xl font-bold tracking-tight text-[#FBF8F0] leading-none">
                Panthera Corbett
              </div>
              <div className="text-[11px] font-sans text-[#8A9468] tracking-wider uppercase mt-1">
                Safari & Forest Night Stays
              </div>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center gap-7">
            {navLinks.map((link) => (
              <Link
                key={link.label}
                href={link.href}
                className="text-sm font-medium text-[#E8E0CC]/90 hover:text-white transition-colors"
              >
                {link.label}
              </Link>
            ))}
          ...
          <div className="hidden sm:flex items-center gap-3">
            <Button
              ...
              size="sm"
              href={`tel:${phoneRaw}`}
              className="text-[#FBF8F0] border-[#8A9468]/60 hover:bg-[#37482E]"
            >
              <svg
                className="w-3.5 h-3.5 mr-2 text-[#C99A3D]"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
      ...
      {/* Mobile Drawer */}
      {mobileMenuOpen && (
        <div className="lg:hidden bg-[#17211A] border-b border-[#37482E] px-4 pt-3 pb-6 space-y-3">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              onClick={() => setMobileMenuOpen(false)}
              className="block py-2 text-base font-medium text-[#E8E0CC] hover:text-white border-b border-[#37482E]/50"
            >
              {link.label}
            </Link>
          ))}
